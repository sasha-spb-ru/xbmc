...
bool CWinSystemAndroidGLESContext::SetHDR(const VideoPicture* videoPicture)
{
  ...
  if (m_hasHDRConfig && m_hasEGL_BT2020_PQ_Colorspace_Extension && m_hasEGL_ST2086_Extension)
  {
    ...
    // some HDR10 streams carry ST.2084 transfer without either.
    // Decide BT2020 PQ tagging from transfer characteristic or Dolby Vision,
    // never from metadata presence or colour matrix.
